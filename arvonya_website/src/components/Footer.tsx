        ...
        <div className="mx-auto max-w-7xl px-6 py-16 grid md:grid-cols-3 gap-10">
          <div>
            ...
            <p className="mt-5 text-sm leading-relaxed text-neutral-400 max-w-xs">
              {tr("Kurumsal güven, kalıcı istikrar ilkesiyle çok sektörlü hizmet sunan stratejik bir grup.", lang)}
            </p>
          </div>
          <div>
            <h4 className="text-xs uppercase tracking-widest text-white mb-4">{tr("İletişim", lang)}</h4>
            <div className="space-y-3 text-sm text-neutral-400 leading-relaxed">
              <p className="flex gap-2 items-start"><MapPin className="h-4 w-4 mt-0.5 shrink-0 text-[#E8521A]" /><span>{COMPANY.address}</span></p>
              <p className="flex gap-2 items-center"><Phone className="h-4 w-4 text-[#2EAA4A]" /><a href={`tel:+${COMPANY.phoneIntl}`} className="text-white hover:underline">{COMPANY.phone}</a></p>
            </div>
          </div>
          <div>
            <h4 className="text-xs uppercase tracking-widest text-white mb-4">{tr("Bizi Takip Edin", lang)}</h4>
            <div className="flex gap-3">
              {[Instagram, Linkedin, Facebook].map((Ic, i) => (
                <a key={i} href="#" className="p-2.5 rounded-full border-2 border-white/15 hover:border-[#E8521A] hover:text-[#E8521A] transition" aria-label="social">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/50 backdrop-blur-md" onClick={() => setModal(null)}>
            <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} onClick={e => e.stopPropagation()} className="w-full max-w-2xl max-h-[80vh] overflow-y-auto bg-white rounded-3xl p-8 md:p-10 shadow-2xl">
              ...
              <div className="prose prose-sm max-w-none text-foreground/80 leading-relaxed space-y-4">
                ...
                <p>Kişisel verileriniz; size hizmet sunmak, iletişim kurmak, talepleri yönetmek ve yasal yükümlülükleri yerine getirmek amacıyla işlenmektedir.</p>
                <p>Verileriniz; üçüncü kişilerle yalnızca açık rızanız doğrultusunda veya yasal zorunluluk halinde paylaşılır. KVKK'nın 11. maddesi gereği her zaman bilgi talep etme, düzeltme, silme ve itiraz haklarına sahipsiniz.</p>
                <p>Detaylı bilgi ve başvuru için {COMPANY.phone} numarasından bize ulaşabilirsiniz.</p>
              </div>
            </motion.div>
          </motion.div>
        )}
