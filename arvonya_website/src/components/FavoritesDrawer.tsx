          ...
          >
            ...
            {items.length === 0 ? (
              <div className="text-center py-20">
                ...
                <p className="text-sm text-muted-foreground max-w-xs mx-auto">Henüz favori mülkünüz bulunmuyor. Keşfetmeye başlayın!</p>
              </div>
            ) : (
              <div className="space-y-3">
                {items.map(item => (
                  <div key={item.id} className="flex gap-3 p-2 rounded-2xl border border-border hover:bg-secondary/40 transition">
                    <Link to="/listing/$id" params={{ id: item.id }} onClick={onClose} className="flex-shrink-0">
                      <img src={item.images[0]} alt="" className="h-16 w-20 object-cover rounded-xl" />
                    </Link>
                    <div className="flex-1 min-w-0">
                      <Link to="/listing/$id" params={{ id: item.id }} onClick={onClose}>
                        <p className="text-sm font-medium truncate">{item.title}</p>
                        <p className="text-xs text-muted-foreground">{item.code}</p>
                        <p className="text-sm font-semibold mt-1">{formatPrice(item.priceTL, currency)}</p>
                      </Link>
                    </div>
                    <button onClick={() => toggleFavorite(item.id)} className="p-2 self-start text-brand-orange">
