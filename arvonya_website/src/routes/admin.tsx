...
import { BarChart3, Home, Car, Languages, CalendarCheck, Inbox, MessageCircle, Layers, Check, Pencil, Trash2, X, Upload, Menu } from "lucide-react";
...
function AdminInner() {
  ...
  const items: { id: Tab; label: string; icon: typeof Home; n: number }[] = [
    ...
    { id: "translations", label: "Tercüme Talepleri", icon: Languages, n: translationRequests.length },
    { id: "appointments", label: "Randevular", icon: CalendarCheck, n: appointments.length },
  ];

  const sidebarBody = (dark: boolean) => (
    <>
      <Link to="/" onClick={() => setMobileOpen(false)}><Logo size="text-lg" /></Link>
      <nav className="mt-10 space-y-1">
        {items.map(({ id, label, icon: Icon, n }) => {
          const active = tab === id;
          const baseLight = active ? "bg-[#1A1A1A] text-white" : "hover:bg-secondary text-[#1A1A1A]";
          const baseDark  = active ? "bg-white text-[#1A1A1A]" : "text-white/80 hover:bg-white/10 hover:text-white";
          return (
            <button key={id} onClick={() => { setTab(id); setMobileOpen(false); }} className={`w-full flex items-center justify-between gap-2 px-3 py-2.5 rounded-xl text-sm font-medium transition ${dark ? baseDark : baseLight}`}>
              <span className="flex items-center gap-2.5 min-w-0"><Icon className="h-4 w-4 shrink-0" /> <span className="truncate">{label}</span></span>
              {n > 0 && <span className={`text-[10px] px-2 py-0.5 rounded-full shrink-0 ${active ? (dark ? "bg-[#1A1A1A]/10" : "bg-white/20") : (dark ? "bg-white/10" : "bg-secondary")}`}>{n}</span>}
            </button>
          );
        })}
      </nav>
      <p className={`mt-auto pt-6 text-[10px] leading-relaxed ${dark ? "text-white/50" : "text-muted-foreground"}`}>{COMPANY.address}</p>
    </>
  );
  ...
  return (
    <div className="min-h-screen flex bg-[#F4F0EA] w-full overflow-x-hidden">
      ...
      <main className="flex-1 p-5 pt-20 md:p-10 md:pt-10 overflow-y-auto w-full min-w-0" style={{ boxSizing: "border-box" }}>
        <button
        ...
        {tab === "stats" && <Stats counts={{ properties: properties.length, vehicles: vehicles.length, translations: translationRequests.length, appointments: appointments.length }} />}
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
        {cards.map(c => (
          <div key={c.label} className="bg-white rounded-2xl p-5 md:p-6 border border-border shadow-sm flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="h-10 w-10 rounded-xl flex items-center justify-center" style={{ backgroundColor: c.bg }}>
                <c.icon className="h-5 w-5" style={{ color: c.color }} />
              </div>
              <span className="text-3xl md:text-4xl font-black" style={{ color: c.color }}>{c.val}</span>
            </div>
            <div>
              <div className="w-full h-1.5 rounded-full bg-neutral-100 overflow-hidden">
                <div className="h-full rounded-full transition-all" style={{ width: `${Math.min(100, (c.val / 20) * 100)}%`, backgroundColor: c.color }} />
              </div>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-3xl font-bold text-[#1A1A1A]">{kind === "property" ? "Emlak İlanları" : "Araç İlanları"}</h1>
        <button onClick={() => setCreating(true)} className="btn-outline-green">+ Yeni İlan</button>
      </div>
      ...
      <div className="hidden md:block bg-white rounded-3xl border border-border overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-secondary/60 text-left">
            <tr>
              {["Başlık", "Kod", "Fiyat", "İşlemler"].map(h => <th key={h} className="px-5 py-3 font-semibold text-[#1A1A1A]">{h}</th>)}
            </tr>
          </thead>
              ...
              <tr key={l.id}>
                ...
                <td className="px-5 py-4">
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setConfirmDel({ id: l.id, title: l.title })} className="btn-outline-red"><Trash2 className="h-3 w-3" /> Sil</button>
                  </div>
                </td>
              </tr>
            ))}
            {list.length === 0 && (
              <tr><td colSpan={4} className="px-5 py-16 text-center text-muted-foreground">Henüz bir ilan bulunmuyor.</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      <div className="md:hidden space-y-3">
        {list.length === 0 && (
          <div className="bg-white rounded-2xl border border-border p-10 text-center text-sm text-muted-foreground">Henüz bir ilan bulunmuyor.</div>
        )}
        {list.map(l => (
      ...
      <AnimatePresence>
        ...
        {confirmDel && (
          <ConfirmDelete
            ...
            onConfirm={() => { deleteListing(kind, confirmDel.id); setConfirmDel(null); }}
          />
        )}
      </AnimatePresence>
    </div>
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm" onClick={onCancel}>
      <motion.div initial={{ scale: 0.94, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.94, y: 20 }} onClick={e => e.stopPropagation()} className="w-full max-w-md bg-white rounded-3xl p-8 shadow-2xl">
        ...
        <div className="flex gap-3 justify-end">
          <button onClick={onCancel} className="px-5 py-2.5 rounded-full border-2 border-border hover:border-[#1A1A1A] text-sm font-medium transition">Vazgeç</button>
          <button onClick={onConfirm} className="px-5 py-2.5 rounded-full bg-red-600 hover:bg-red-700 text-white text-sm font-semibold transition">Evet, Sil</button>
        </div>
      </motion.div>
    </motion.div>
  );
...
function ListingModal({ kind, initial, onClose }: { kind: "property" | "vehicle"; initial: Property | Vehicle | null; onClose: () => void }) {
  ...
  const [p, setP] = useState({
    listingType: (init && (init as Property).kind === "property") ? (init as Property).listingType : "sale" as "sale" | "rent",
    subType: (init && (init as Property).kind === "property") ? (init as Property).subType : "konut" as "konut" | "arsa",
  ...
  const save = () => {
    if (!common.title || !common.priceTL) { alert("Başlık ve fiyat zorunludur."); return; }
    const base = {
    ...
    if (kind === "property") {
      next = {
        ...base, kind: "property", listingType: p.listingType, subType: p.subType,
        location: p.location, type: p.type,
      ...
      next = { ...base, kind: "vehicle", listingType: "sale", year: Number(v.year) || new Date().getFullYear(), km: Number(v.km) || 0, gear: v.gear, fuel: v.fuel, damage: v.damage } as Vehicle;
    }
    if (isEdit) updateListing(next); else addListing(next);
    onClose();
  };
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} onClick={e => e.stopPropagation()} className="w-full max-w-2xl max-h-[90vh] overflow-y-auto bg-white rounded-3xl p-8 shadow-2xl">
        <div className="flex items-center justify-between mb-6">
          <h3 className="text-xl font-bold text-[#1A1A1A]">{isEdit ? "İlanı Düzenle" : "Yeni İlan"}</h3>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-secondary"><X className="h-4 w-4" /></button>
        </div>

        <div className="space-y-4">
          <input className="w-full px-4 py-3 rounded-xl border-2 border-border focus:border-[#1A1A1A] outline-none" placeholder="Başlık" value={common.title} onChange={e => setCommon({ ...common, title: e.target.value })} />
          <input className="w-full px-4 py-3 rounded-xl border-2 border-border focus:border-[#1A1A1A] outline-none" placeholder="Fiyat (TL)" value={common.priceTL} onChange={e => setCommon({ ...common, priceTL: e.target.value })} />
          <textarea className="w-full px-4 py-3 rounded-xl border-2 border-border focus:border-[#1A1A1A] outline-none" placeholder="Açıklama" rows={3} value={common.description} onChange={e => setCommon({ ...common, description: e.target.value })} />
          ...
          {kind === "property" ? (
            <div className="grid grid-cols-2 gap-3">
              <select className="px-4 py-3 rounded-xl border-2 border-border" value={p.listingType} onChange={e => setP({ ...p, listingType: e.target.value as "sale" | "rent" })}>
                <option value="sale">Satılık</option><option value="rent">Kiralık</option>
              </select>
              <select className="px-4 py-3 rounded-xl border-2 border-border" value={p.subType} onChange={e => setP({ ...p, subType: e.target.value as "konut" | "arsa" })}>
                <option value="konut">Konut / Daire</option><option value="arsa">Arsa / Bahçe</option>
              </select>
              <input className="px-4 py-3 rounded-xl border-2 border-border col-span-2" placeholder="Lokasyon (İl / İlçe / Mahalle)" value={p.location} onChange={e => setP({ ...p, location: e.target.value })} />
              {p.subType === "konut" ? (
                <>
                  <input className="px-4 py-3 rounded-xl border-2 border-border" placeholder="Oda sayısı (3+1)" value={p.rooms} onChange={e => setP({ ...p, rooms: e.target.value })} />
                  <input className="px-4 py-3 rounded-xl border-2 border-border" placeholder="m²" value={p.m2} onChange={e => setP({ ...p, m2: e.target.value })} />
                  <input className="px-4 py-3 rounded-xl border-2 border-border" placeholder="Banyo" value={p.baths} onChange={e => setP({ ...p, baths: e.target.value })} />
                  <input className="px-4 py-3 rounded-xl border-2 border-border" placeholder="Bulunduğu Kat" value={p.floor} onChange={e => setP({ ...p, floor: e.target.value })} />
                </>
              ) : (
                <>
                  <input className="px-4 py-3 rounded-xl border-2 border-border" placeholder="Toplam Alan (m²)" value={p.area} onChange={e => setP({ ...p, area: e.target.value })} />
                  <select className="px-4 py-3 rounded-xl border-2 border-border" value={p.tapu} onChange={e => setP({ ...p, tapu: e.target.value })}>
                    <option>Müstakil</option><option>Hisseli</option>
                  </select>
                  <select className="px-4 py-3 rounded-xl border-2 border-border col-span-2" value={p.imar} onChange={e => setP({ ...p, imar: e.target.value })}>
                    <option>Konut İmarlı</option><option>Tarla</option><option>Bağ-Bahçe</option>
                  </select>
                </>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <input className="px-4 py-3 rounded-xl border-2 border-border" placeholder="Yıl" value={v.year} onChange={e => setV({ ...v, year: e.target.value })} />
              <input className="px-4 py-3 rounded-xl border-2 border-border" placeholder="Kilometre" value={v.km} onChange={e => setV({ ...v, km: e.target.value })} />
              <select className="px-4 py-3 rounded-xl border-2 border-border" value={v.gear} onChange={e => setV({ ...v, gear: e.target.value })}><option>Otomatik</option><option>Manuel</option></select>
              <select className="px-4 py-3 rounded-xl border-2 border-border" value={v.fuel} onChange={e => setV({ ...v, fuel: e.target.value })}><option>Benzin</option><option>Dizel</option><option>Hibrit</option><option>Elektrik</option></select>
              <select className="px-4 py-3 rounded-xl border-2 border-border col-span-2" value={v.damage} onChange={e => setV({ ...v, damage: e.target.value })}><option>Hasarsız</option><option>Az Hasarlı</option><option>Ekspertizli</option></select>
            </div>
          )}
...
function SectorsManage() {
  ...
  return (
    <div>
      ...
      <p className="text-sm text-muted-foreground mb-8">Pasif yapılan sektörler ana navigasyondan otomatik olarak gizlenir.</p>
      {/* Desktop table */}
      <div className="hidden md:block bg-white rounded-3xl border border-border overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-secondary/60 text-left">
            <tr>{["Sektör", "Açıklama", "Durum", "İşlemler"].map(h => <th key={h} className="px-5 py-3 font-semibold text-[#1A1A1A]">{h}</th>)}</tr>
          </thead>
          <tbody className="divide-y divide-border">
            {sectors.map(s => (
              <tr key={s.id}>
...
function SectorEditModal({ sector, onClose }: { sector: Sector; onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/40 backdrop-blur-md" onClick={onClose}>
      <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20 }} onClick={e => e.stopPropagation()} className="w-full max-w-lg bg-white rounded-3xl p-8 shadow-2xl">
        ...
        <div className="space-y-4">
          <label className="block">
            ...
            <input value={label} onChange={e => setLabel(e.target.value)} className="mt-1.5 w-full px-4 py-3 rounded-xl border-2 border-border focus:border-[#1A1A1A] outline-none" />
          </label>
          <label className="block">
            <span className="text-xs uppercase tracking-widest text-muted-foreground">Açıklama</span>
            <textarea value={description} onChange={e => setDescription(e.target.value)} rows={4} className="mt-1.5 w-full px-4 py-3 rounded-xl border-2 border-border focus:border-[#1A1A1A] outline-none" />
          </label>
          <div className="flex gap-3 justify-end pt-2">
            <button onClick={onClose} className="px-5 py-2.5 rounded-full border-2 border-border hover:border-[#1A1A1A] text-sm font-medium transition">İptal</button>
...
function RequestsTable({ type }: { type: "translations" | "appointments" }) {
  ...
  if (rows.length === 0) {
    return (
      <div>
        <h1 className="text-3xl font-bold mb-8 text-[#1A1A1A]">{type === "appointments" ? "Randevular" : "Tercüme Talepleri"}</h1>
  ...
  return (
    <div>
      ...
      <div className="bg-white rounded-3xl border border-border overflow-hidden shadow-sm">
        <table className="w-full text-sm">
          <thead className="bg-secondary/60 text-left">
            <tr>{(type === "appointments" ? ["Ad", "Telefon", "Tarih", "İlan", "Durum", ""] : ["Ad", "E-posta", "Dil", "Dosya", ""]).map(h => <th key={h} className="px-5 py-3 font-semibold text-[#1A1A1A]">{h}</th>)}</tr>
          </thead>
          <tbody className="divide-y divide-border">
            {type === "appointments" ? appointments.map(a => {
