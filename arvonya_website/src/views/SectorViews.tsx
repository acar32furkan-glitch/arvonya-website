...
export function GayrimenkulView() {
  const { properties, sort, listingTypeFilter, searchQuery, locationFilter, lang, priceMin, priceMax, roomsFilter } = useStore();
  ...
  if (searchQuery.trim()) {
    ...
    filtered = filtered.filter(p => p.title.toLowerCase().includes(q) || p.location.toLowerCase().includes(q) || p.code.toLowerCase().includes(q));
  }
  ...
  if (roomsFilter) filtered = filtered.filter(p => p.kind === "property" && p.rooms === roomsFilter);
    ...
    <motion.section {...fade}>
      ...
      <div className="mx-auto max-w-7xl px-6 pb-20">
        <div className="space-y-10">
          {sorted.length > 0 ? sorted.map(p => <ListingCard key={p.id} item={p} />) : (
    ...
    <motion.section {...fade}>
      <div className="mx-auto max-w-7xl px-6 pt-32 pb-16">
        <header className="mb-12 md:mb-16">
          <p className="text-xs uppercase tracking-[0.3em] text-brand-orange mb-3">{tr("Otomotiv", lang)}</p>
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight max-w-3xl leading-[1.05]">
            <span className="text-[#E8521A]">{tr("Güvenilir", lang)}</span> {tr("Araçlar,", lang)}
            <br />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {trades.map(t => (
          <a key={t.title} href={waUrl(t.title)} target="_blank" rel="noreferrer" className="group relative aspect-[4/5] overflow-hidden rounded-3xl bg-card border border-border/60 block">
            <img src={t.image} alt={t.title} className="absolute inset-0 h-full w-full object-cover transition-all duration-700 group-hover:scale-105 group-hover:brightness-[0.35]" />
            <div className="absolute inset-0 bg-gradient-to-t from-black/70 via-black/20 to-transparent" />
            <div className="absolute inset-0 p-7 flex flex-col justify-end text-white">
              <h3 className="text-2xl font-semibold leading-tight">{tr(t.title, lang)}</h3>
...
export function TercumeView() {
  ...
  const [data, setData] = useState({ from: "Türkçe", to: "İngilizce", fileName: "", name: "", email: "", phone: "" });
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      ...
      setLoading(false);
      setQuote("Yapay Zeka Destekli Tahmini Teklif: 1.500 TL – 1.900 TL (Kesin fiyatlandırma uzman onayı sonrası iletilecektir)");
    }, 1800);
  };
  ...
  return (
    <motion.section {...fade} className="mx-auto max-w-3xl px-6 pt-32 pb-24">
      <header className="mb-12 text-center">
        <p className="text-xs uppercase tracking-[0.3em] text-muted-foreground mb-3">Tercüme Hizmetleri</p>
        <h1 className="text-4xl md:text-5xl font-bold tracking-tight">Belgenizi yükleyin, yapay zeka teklifi anında.</h1>
      </header>

      {quote ? (
        <div className="rounded-3xl border border-border bg-card p-10 text-center">
          <div className="mx-auto h-14 w-14 rounded-full bg-brand-green/10 flex items-center justify-center mb-6"><Check className="h-7 w-7 text-brand-green" /></div>
        ...
        <div className="rounded-3xl border border-border bg-card p-8 md:p-10">
          ...
          {step === 1 && (
            <div className="space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4">
                <Select label="Kaynak Dil" value={data.from} onChange={v => setData({ ...data, from: v })} />
                <Select label="Hedef Dil" value={data.to} onChange={v => setData({ ...data, to: v })} />
              </div>
              <button onClick={next} className="w-full py-3 rounded-full bg-foreground text-background font-medium">Devam</button>
            </div>
          )}
          ...
          {step === 2 && (
            <div className="space-y-5">
              ...
              <label className="block">
                <input type="file" className="hidden" onChange={e => setData({ ...data, fileName: e.target.files?.[0]?.name ?? "" })} />
                <div className="border-2 border-dashed border-border rounded-2xl p-10 text-center hover:bg-secondary/50 cursor-pointer transition">
                  ...
                  <p className="text-sm">{data.fileName || "Dosyayı buraya sürükleyin veya tıklayın"}</p>
                </div>
              </label>
              {data.fileName && <div className="h-1 bg-border rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: "100%" }} transition={{ duration: 1.2 }} className="h-full bg-brand-green" /></div>}
              <button onClick={next} disabled={!data.fileName} className="w-full py-3 rounded-full bg-foreground text-background font-medium disabled:opacity-40">Devam</button>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <input className="w-full px-4 py-3 rounded-xl border border-border" placeholder="Ad Soyad" value={data.name} onChange={e => setData({ ...data, name: e.target.value })} />
              <input className="w-full px-4 py-3 rounded-xl border border-border" placeholder="E-posta" value={data.email} onChange={e => setData({ ...data, email: e.target.value })} />
              <input className="w-full px-4 py-3 rounded-xl border border-border" placeholder="Telefon" value={data.phone} onChange={e => setData({ ...data, phone: e.target.value })} />
              <button onClick={submit} disabled={loading} className="w-full py-3 rounded-full bg-foreground text-background font-medium flex items-center justify-center gap-2">
                {loading ? <><Loader2 className="h-4 w-4 animate-spin" /> Analiz ediliyor…</> : "Ücretsiz Teklif Al"}
              </button>
            </div>
          )}
        </div>
...
function Select({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  ...
  return (
    <label className="block">
      ...
      <select value={value} onChange={e => onChange(e.target.value)} className="mt-1.5 w-full px-4 py-3 rounded-xl border border-border bg-white">
    ...
    <motion.section {...fade} className="mx-auto max-w-4xl px-6 pt-32 pb-24">
      ...
      <div className="rounded-3xl border border-border bg-card p-2">
        <div className="flex gap-1 overflow-x-auto p-1 bg-secondary/50 rounded-2xl">
          {tabs.map(t => (
            <button key={t.id} onClick={() => setActive(t.id)} className={`flex-1 min-w-fit px-5 py-2.5 text-sm font-medium rounded-xl transition ${active === t.id ? "bg-white shadow-sm" : "text-muted-foreground"}`}>{t.label}</button>
          ))}
        </div>
        <AnimatePresence mode="wait">
          <motion.div key={active} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.3 }} className="p-8 md:p-12">
            <p className="text-lg md:text-xl leading-relaxed text-foreground/80">{current.content}</p>
          </motion.div>
        </AnimatePresence>
      </div>
    </motion.section>
